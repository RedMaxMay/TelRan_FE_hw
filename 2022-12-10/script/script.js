...
// writeInLocal получает в качестве аргумента массив и записывает
// его в localStorage под ключем tasks

function writeInLocal(arr) {
  localStorage.setItem("products", JSON.stringify(arr));
}

// readFromLocal не получает аргументов и возвращает из localStorage
...
addProd.addEventListener("submit", (event) => {
  ...
  writeInLocal([
    ...readFromLocal(),
    { title, price, quantity, id: Date.now() },
  ]);
...
function reRender(arr) {
  ...
  if (readFromLocal().length === 0) {
    const par = document.createElement("p");
    par.innerText = `There are no Products...`;
    products.append(par);
  } else {
    ...
    arr.forEach(({ title, price, quantity, id }) => {
    ...
    >
      ...
      productItem.addEventListener("click", (event) => {
        if (event.target.closest("svg")) {
          writeInLocal(readFromLocal().filter((elem) => elem.id !== id));
          reRender(readFromLocal());
        }
      });
...
// Нужно ли функцию reRender разделять и делать
// несколько дополнительных "подфункций?"
